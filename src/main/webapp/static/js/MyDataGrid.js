var MyDataGrid = function(config){
		config = config || {};
		...
		var Grid = $('#data-list');
		//Form
		var Form = {search:$("#searchForm"),edit:$("#editForm")}
		//Win 窗口
		...
		var initGrid = function(){
			var dataconfig = {
				...
				queryParams: Form.search.serializeObject(),//传递参数（*）
				pageSize:dataGrid.pageSize||20,//每页显示的记录条数，默认为10
				...
				onLoadSuccess: dataGrid.onLoadSuccess || function(){
					...
					if(total==0){
						$('.datagrid-view').css('height','200px');					 
						var noData="<div class='noData-tip'>很抱歉，暂时没有数据</div>";
						$('.datagrid-view1 .datagrid-body').hide();
						$('.datagrid-view2 .datagrid-body').css({'overflow':'hidden','height':'150px'}).html('').append(noData);
						}
					else{
						$('.datagrid-view1 .datagrid-body').show();
