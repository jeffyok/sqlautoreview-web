...
public class ParseMutiTableSQL extends ParseSQL {
	...
	public int AnalyzeWhereStr(String whereString)
	{
		...
		if(list_Table_Relationship.size()>1){
	...
	private String findConnectColumn(String whereString) {
		...
		for(int i=0;i<columnStrings.length;i++)
		{
			...
			//修改，添加sub_columnStrings!=null && sub_columnStrings.length>=3条件
			logger.info("===whereString====="+whereString);
			if(sub_columnStrings!=null && sub_columnStrings.length>=3 && sub_columnStrings[1].equals("=") && sub_columnStrings[2].equals("1"))
			{
