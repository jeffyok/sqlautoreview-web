...
public class CreateIndex {
    ...
    //构造函数
	public CreateIndex(ProjectModel projectModel)
	{
		this.md = new MySQLMetaData(projectModel);
	}
	...
	//分析每一个SQL
	public void reviewSQL(List<SqlreviewModel> sqlreviewList,SqlreviewMapper mapper)
	{
		Date now = new Date();
		String sql;
		int sql_id;
		...
		for(SqlreviewModel sqlReview:sqlreviewList)
    	{
			auto_review_error="";
			auto_review_tip="";
			tablenames="";
    		sql = sqlReview.getRealSql();
    		sql_id =  sqlReview.getId();
    		logger.info("待审核的SQL_ID="+sql_id+",原始SQL TEXT为:"+sql);
    		ParseSQL ps = new ParseSQL(sql);
    		ps.sql_dispatch();
    		//检查sql parse的结果
    		auto_review_tip=ps.tip;
    		if(checkParseResult(ps.errmsg,ps.sql_type,sql_id,mapper)==false){
    			continue;
    		}
    		...
    		if(ps.tag==0)
    		{
    			    ...
    			    //单表填冲元数据
    			    if(fillTableMetaData(ps.tablename,sql_id,mapper)==false){
    			    	continue;
    			    }
		    		...
		    		try {
		        		...
		        		if(auto_review_error.length()>0){
		        			SqlreviewModel sqlreviewModel = new SqlreviewModel();
		        			sqlreviewModel.setId(sql_id);
		        			sqlreviewModel.setStatus(2);
		        			sqlreviewModel.setAutoReviewTime(now);
		        			sqlreviewModel.setUpdateTime(now);
		        			sqlreviewModel.setSqlAutoIndex("");
		        			sqlreviewModel.setAutoReviewErr(auto_review_error);
		        			sqlreviewModel.setAutoReviewTip(auto_review_tip);
		        			sqlreviewModel.setTableName(tablenames);
		        			mapper.updateBySelective(sqlreviewModel);
		        			continue;
		        		}
		        		...
		        		SqlreviewModel sqlreviewModel = new SqlreviewModel();
	        			sqlreviewModel.setId(sql_id);
	        			sqlreviewModel.setStatus(1);
	        			sqlreviewModel.setAutoReviewTime(now);
	        			sqlreviewModel.setUpdateTime(now);
	        			sqlreviewModel.setSqlAutoIndex(createindexString);
	        			sqlreviewModel.setAutoReviewErr(auto_review_error);
	        			sqlreviewModel.setAutoReviewTip(auto_review_tip);
	        			sqlreviewModel.setTableName(tablenames);
	        			mapper.updateBySelective(sqlreviewModel);
    			...
    			for(Iterator<ParseStruct> iter=pmts.list_ParseStruct.iterator();iter.hasNext();)
    			{
    			    ...
    			    if(fillTableMetaData(tmp_ps.tablename,sql_id,mapper)==false){
    			...
    			sqlreviewModel.setAutoReviewTime(now);
    			sqlreviewModel.setUpdateTime(now);
    			sqlreviewModel.setSqlAutoIndex(createindexString2);
    			sqlreviewModel.setAutoReviewErr(auto_review_error);
    			sqlreviewModel.setAutoReviewTip(auto_review_tip);
    			sqlreviewModel.setTableName(tablenames);
    			mapper.updateBySelective(sqlreviewModel);
	...
	private void LoadWhereDataToACS(String whereString,AnalyzeTableStructure ats) 
	{
		...
		{
			    String column_name=Columns[i].substring(0, Columns[i].indexOf(":")).replace("`", "");
	...
	private boolean checkParseResult(String errmsg, int sql_type,int sql_id,SqlreviewMapper mapper) 
	{
		Date now = new Date();
		if(errmsg.length()>0)
		{
	 ...
	 * 填冲单表的元数据
	 */
	private boolean fillTableMetaData(String tablename,int sql_id,SqlreviewMapper mapper) 
	{
		Date now = new Date();
		//获得最相近的表名,支持分库分表
		String real_tablename=md.findMatchTable(tablename);
	...
	private void LoadWhereDataToACS(Tree_Node whereNode,
		...
		else if(whereNode.node_type==2)
		{
			...
			//左孩子肯定为条件变量列名
			acs.column_name=whereNode.left_node.node_content.replace("`", "");
			//查找这一列的元数据
			Column_Node cn=md.searchColumnMetaData(ats.tablename, acs.column_name);
			if(cn==null){
				if(!"1".equals(acs.column_name)){
					auto_review_error="Table:"+ats.tablename+" Column:"+acs.column_name+" does not exist.";
					logger.warn(auto_review_error);
				}
				return;
